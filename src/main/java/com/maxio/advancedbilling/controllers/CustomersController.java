...
public final class CustomersController extends BaseController {
     ...
     * restriction is that you can only create one customer for a given reference value. If
     * provided, the `reference` value must be unique. It represents a unique identifier for the
     * customer from your own app, i.e. the customer’s ID. This allows you to retrieve a given
     * customer via a piece of shared information. Alternatively, you can choose to leave
     * `reference` blank, and store the system-assigned unique ID for the customer, which is in the
     * `id` attribute. For more information, see [Customer
     * Details](https://maxio.zendesk.com/hc/en-us/articles/24252190590093-Customer-Details). ##
     * Required Country Format Format the country attribute of the customer using the ISO Standard
     * Country codes. Countries should be formatted as two characters. For more information, see
     * [ISO 3166-1](http://en.wikipedia.org/wiki/ISO_3166-1#Current_codes). ## Required State Format
     * Format the state attribute of the customer using the ISO Standard State codes. + US States
     * (two characters): see [ISO 3166-2](https://en.wikipedia.org/wiki/ISO_3166-2:US). + States
     * Outside the US (two to three characters): To find the correct state codes outside the US, go
     * to [ISO 3166-1](http://en.wikipedia.org/wiki/ISO_3166-1#Current_codes) and click on the link
     * in the “ISO 3166-2 codes” column next to the country you wish to populate. ## Locale You can
     * attribute a language/region to the customer to deliver invoices in any required language. For
     ...
     * ## Find Customer Use the search feature with the `q` query parameter to retrieve an array of
     * customers that matches the search query. Common use cases are: + Search by an email + Search
     * by an Advanced Billing ID + Search by an organization + Search by a reference value from your
     * application + Search by a first or last name To retrieve a single, exact match by reference,
     * use the [lookup
    ...
    /**
     * Lists all subscriptions that belong to a customer. If you have the new [Catalog
