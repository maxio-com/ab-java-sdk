...
public final class AdvanceInvoiceController extends BaseController {
    ...
    /**
     * Issues an invoice in advance for a subscription's next renewal date. For the most part,
     * advance invoices function like any other invoice, except they are issued early and have
     * special behavior upon being voided. For more information on advance invoices, including
     * eligibility for generating one, see [Issue Invoice In
     ...
     * advance invoice when one already exists returns an error. Regeneration of the invoice can be
     * forced with the params `force: true`, which voids an advance invoice if one exists and
     * generates a new one. If no advance invoice exists, a new one is generated. Consider using
     * either the create or preview endpoints for proforma invoices to preview this advance invoice
     * before using this endpoint to generate it.
    ...
    /**
     * Returns the advance invoice generated for a subscription's upcoming renewal. There can only
     * be one advance invoice per subscription per billing cycle.
     * @param  subscriptionId  Required parameter: The Chargify id of the subscription.
     * @return    Returns the Invoice response from the API call
     * @throws    ApiException    Represents error response from the server.
     * @throws    IOException    Signals that an I/O exception of some sort has occurred.
     ...
     * desired. A `reason` is required to void, and the invoice must have an open status. Voiding
     * causes any prepayments and credits that were applied to the invoice to be returned to the
     * subscription. For a full overview of the impact of voiding, see [Invoice]($m/Invoice).
     * @param  subscriptionId  Required parameter: The Chargify id of the subscription.
     * @param  body  Optional parameter:
     * @return    Returns the Invoice response from the API call
     * @throws    ApiException    Represents error response from the server.
