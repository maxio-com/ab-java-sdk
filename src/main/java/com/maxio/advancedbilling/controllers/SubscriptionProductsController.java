...
public final class SubscriptionProductsController extends BaseController {
    ...
    /**
     * Migrates a subscription to a different product. In order to create a migration, you must pass
     * the `product_id` or `product_handle` in the object when you send a POST request. You may also
     ...
     * will be moved to the products default price point. The response will be the updated
     * subscription. ## Valid Subscriptions Subscriptions should be in the `active` or `trialing`
     * state in order to be migrated. (For backwards compatibility reasons, it is possible to
     * migrate a subscription that is in the `trial_ended` state via the API, however this is not
     * recommended. Since `trial_ended` is an end-of-life state, the subscription should be
     * canceled, the product changed, and then the subscription can be reactivated.) ## Migrations
     * Documentation Full documentation on how to record Migrations in the Advanced Billing UI can
     * be located
