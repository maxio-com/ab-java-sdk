...
public final class EventsController extends BaseController {
     ...
     * "Events" are referred to as "Site Activity". See the [Site
     * Activity](https://maxio.zendesk.com/hc/en-us/articles/24250671733517-Site-Activity) article
     * in the product documentation for details on how to record view Events / Site Activty in the
     * Advanced Billing UI. If you’re using the [enhanced Catalog
     ...
     * changes to Product on Subscription” ## List Events for a Site This method will retrieve a
     * list of events for a site. Use query string filters to narrow down results. You may use the
     * `key` filter as part of your query string to narrow down results. ### Legacy Filters The
     * following keys are no longer supported. + `payment_failure_recreated` +
     * `payment_success_recreated` + `renewal_failure_recreated` + `renewal_success_recreated` +
     * `zferral_revenue_post_failure` - (Specific to the deprecated Zferral integration) +
     * `zferral_revenue_post_success` - (Specific to the deprecated Zferral integration) ## Event
     * Key The event type is identified by the key property. You can check supported keys
     * [here]($m/Event%20Key). ## Event Specific Data Different event types may include additional
     * data in `event_specific_data` property. While some events share the same schema for
     * `event_specific_data`, others may not include it at all. For precise mappings from key to
     * event_specific_data, refer to [Event]($m/Event). ### Example Here’s an example event for the
     * `subscription_product_change` event: ``` { "event": { "id": 351, "key":
     * "subscription_product_change", "message": "Product changed on Mark Alan's subscription from
     ...
     * "subscription_state_change", "message": "State changed on Mark Alan's subscription to Pro
     ...
     * property. You can check supported keys [here]($m/Event%20Key). If you’re using the [enhanced
     * Catalog
     * experience](page:help/announcements/2026-announcements#new-catalog-experience-and-terminology),
     * you’ll see updated naming in webhook events and messages. Event name changes: -
     * subscription_product_change → subscription_plan_change - component_allocation_change →
     ...
     * from previous plan to new plan” ## Event Specific Data Different event types may include
     * additional data in `event_specific_data` property. While some events share the same schema
     * for `event_specific_data`, others may not include it at all. For precise mappings from key to
