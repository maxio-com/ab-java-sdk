...
public final class SubscriptionStatusController extends BaseController {
     ...
     * see [Subscriber Interface
     * Overview](https://maxio.zendesk.com/hc/en-us/articles/24252493695757-Subscriber-Interface-Overview).
     * ## Optional Component Fields This endpoint is particularly useful because it returns the
     * computed billing amount for the base product and the components which are in use by a
     * subscriber. By default, the preview includes billing details for all components _at their
     * **current** quantities_. This means: * Current `allocated_quantity` for quantity-based
     ...
     * value as of the call to the renewal preview endpoint. End-of-period values for components are
     * not predicted, so metered or events-based usage may be less than it will eventually be at the
     * end of the period. Optionally, **you can provide your own custom quantities** for any
     ...
     * documentation below. ## Preview Behavior Sending a `POST` request to this endpoint returns
     * preview data without modifying the subscription. This method previews data, but does not log
