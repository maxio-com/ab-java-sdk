...
public class OffersControllerUnArchiveTest {
    ...
    @Test
    void shouldUnArchiveOffer() throws ApiException, IOException {
        // given
        String handle = "test-handle-unarchive-" + randomAlphabetic(10).toLowerCase();
        String name = "unarchive offer test " + randomAlphabetic(10).toLowerCase();
        ...
        assertThat(offersController.readOffer(existingOffer.getId()).getOffer().getArchivedAt()).isNotNull();

        // when
        offersController.unarchiveOffer(existingOffer.getId());
    ...
    void shouldReturn404IfOfferDoesNotExist() {
        // when - then
        assertNotFound(() -> offersController.unarchiveOffer(1));
    }
    ...
    void shouldThrowExceptionOnInvalidCredentials() throws IOException, ApiException {
        ...
        assertUnauthorized(() -> TestClientProvider.createInvalidCredentialsClient().getOffersController()
                .unarchiveOffer(existingOffer.getId()));
    }
}
