...
| `NextAssessmentAt` | `ZonedDateTime` | Optional | Timestamp that indicates when capture of payment will be tried or retried. This value will usually track the current_period_ends_at, but will diverge if a renewal payment fails and must be retried. In that case, the current_period_ends_at will advance to the end of the next period (time doesn’t stop because a payment was missed) but the next_assessment_at will be scheduled for the auto-retry time (e.g., 24 hours in the future, in some cases). | ZonedDateTime getNextAssessmentAt() | setNextAssessmentAt(ZonedDateTime nextAssessmentAt) |
...
| `SnapDay` | `String` | Optional | A day of month that subscription will be processed on. Can be 1 up to 28 or 'end'. | String getSnapDay() | setSnapDay(String snapDay) |
...
| `ReasonCode` | `String` | Optional | The churn reason code associated to a canceled subscription. | String getReasonCode() | setReasonCode(String reasonCode) |
...
Subscription subscription = new Subscription.Builder()
    ...
    .creditCard(new CreditCardPaymentProfile.Builder(
