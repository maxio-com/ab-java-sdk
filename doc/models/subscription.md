...
| `CurrentPeriodEndsAt` | `LocalDateTime` | Optional | Timestamp relating to the end of the current (recurring) period (i.e.,when the next regularly scheduled attempted charge will occur) | LocalDateTime getCurrentPeriodEndsAt() | setCurrentPeriodEndsAt(LocalDateTime currentPeriodEndsAt) |
| `NextAssessmentAt` | `LocalDateTime` | Optional | Timestamp that indicates when capture of payment will be tried or,retried. This value will usually track the current_period_ends_at, but,will diverge if a renewal payment fails and must be retried. In that,case, the current_period_ends_at will advance to the end of the next,period (time doesn’t stop because a payment was missed) but the,next_assessment_at will be scheduled for the auto-retry time (i.e. 24,hours in the future, in some cases) | LocalDateTime getNextAssessmentAt() | setNextAssessmentAt(LocalDateTime nextAssessmentAt) |
| `TrialStartedAt` | `LocalDateTime` | Optional | Timestamp for when the trial period (if any) began | LocalDateTime getTrialStartedAt() | setTrialStartedAt(LocalDateTime trialStartedAt) |
| `TrialEndedAt` | `LocalDateTime` | Optional | Timestamp for when the trial period (if any) ended | LocalDateTime getTrialEndedAt() | setTrialEndedAt(LocalDateTime trialEndedAt) |
| `ActivatedAt` | `LocalDateTime` | Optional | Timestamp for when the subscription began (i.e. when it came out of trial, or when it began in the case of no trial) | LocalDateTime getActivatedAt() | setActivatedAt(LocalDateTime activatedAt) |
| `ExpiresAt` | `LocalDateTime` | Optional | Timestamp giving the expiration date of this subscription (if any) | LocalDateTime getExpiresAt() | setExpiresAt(LocalDateTime expiresAt) |
| `CreatedAt` | `LocalDateTime` | Optional | The creation date for this subscription | LocalDateTime getCreatedAt() | setCreatedAt(LocalDateTime createdAt) |
| `UpdatedAt` | `LocalDateTime` | Optional | The date of last update for this subscription | LocalDateTime getUpdatedAt() | setUpdatedAt(LocalDateTime updatedAt) |
...
| `CanceledAt` | `LocalDateTime` | Optional | The timestamp of the most recent cancellation | LocalDateTime getCanceledAt() | setCanceledAt(LocalDateTime canceledAt) |
| `CurrentPeriodStartedAt` | `LocalDateTime` | Optional | Timestamp relating to the start of the current (recurring) period | LocalDateTime getCurrentPeriodStartedAt() | setCurrentPeriodStartedAt(LocalDateTime currentPeriodStartedAt) |
...
| `DelayedCancelAt` | `LocalDateTime` | Optional | Timestamp for when the subscription is currently set to cancel. | LocalDateTime getDelayedCancelAt() | setDelayedCancelAt(LocalDateTime delayedCancelAt) |
...
| `ScheduledCancellationAt` | `LocalDateTime` | Optional | - | LocalDateTime getScheduledCancellationAt() | setScheduledCancellationAt(LocalDateTime scheduledCancellationAt) |
